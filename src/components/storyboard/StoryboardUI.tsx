...
export const StoryboardSidebar = ({
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#323232] text-sm overflow-hidden">
      <div className="flex flex-col border-b border-black shrink-0 max-h-[60%] overflow-y-auto custom-scrollbar">
        <div className="bg-[#282828] px-3 py-1.5 text-xs font-bold text-neutral-300 flex items-center gap-2 border-b border-black sticky top-0 z-10 shadow-sm">
        ...
        <div className="p-4 flex flex-col gap-4">
          {(tool === 'pen' || tool === 'eraser') ? (
            <>
              ...
              {tool === 'pen' && (
                <div className="mt-2 pt-4 border-t border-black/50 flex flex-col gap-3">
                  <div className="text-xs text-neutral-400 uppercase tracking-wider font-semibold flex justify-between items-center">
                    <span>Brush Settings</span>
                    {isCustom ? (
                      <button onClick={() => { removeCustomBrush(brushPreset); handleBrushPresetChange('solid'); }} className="text-red-400 hover:text-red-300 flex items-center gap-1 transition-colors"><Trash2 size={12}/> Delete</button>
                    ) : isOverriddenDefault ? (
                      <button onClick={() => { removeCustomBrush(brushPreset); }} className="text-amber-400 hover:text-amber-300 flex items-center gap-1 transition-colors"><Undo size={12}/> Reset</button>
                    ) : null}
                  </div>
